...
void solve(){
    ...
    for(int i{0}; i < m; ++i){
        int a, b;
        cin >> a >> b;        
        mcmf.addEdge(out(a), in(b), 1, 0);
    }
    for(int i{1}; i <= n; ++i){
        mcmf.addEdge(in(i), out(i), 1, -1);
    }
    for(int i{1}; i <= n; ++i){
        mcmf.addEdge(out(source), in(i), 1, 0);
        mcmf.addEdge(out(i), in(sink), 1, 0);
    }
    mcmf.addEdge(in(source), out(source), 2, 0);
    cout << -mcmf.run(in(source), in(sink)).first << "\n";
}
